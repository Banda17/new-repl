...
function generateComparativeLoadingPDF(doc: typeof PDFDocument, data: any) {
  try {
    ...
    const totalCurrent = data.data.reduce((sum: number, row: any) => sum + (row.currentMT || 0), 0);
    const totalPrevious = data.data.reduce((sum: number, row: any) => sum + (row.compareMT || 0), 0);
...
function generateStationComparativeLoadingPDF(doc: typeof PDFDocument, data: any) {
  try {
    // Clean header design matching comparative loading format
    doc.rect(0, 0, 842, 100).fill('#1e3a8a');
    ...
    // Main title - centered with proper spacing
    doc.fillColor('white').fontSize(24).font('Helvetica-Bold')
       .text("STATION WISE COMPARATIVE LOADING", 120, 20, { align: "center", width: 600 });
    
    // Period information on separate lines for clarity
    doc.fontSize(11).font('Helvetica-Bold')
       .text(`Period: ${data.periods.current}`, 120, 50, { align: "center", width: 600 });
    doc.fontSize(10).font('Helvetica')
       .text(`vs ${data.periods.previous}`, 120, 65, { align: "center", width: 600 });
    ...
    // Period section headers with better spacing
    doc.fillColor('black').fontSize(12).font('Helvetica-Bold');
    doc.text(data.periods.current, 180, yPosition, { width: 200, align: 'center' });
    doc.text(data.periods.previous, 400, yPosition, { width: 200, align: 'center' });
    doc.text("Variation in", 620, yPosition, { width: 120, align: 'center' });
    ...
    // Table border
    doc.rect(tableStartX, tableStartY, tableWidth, 25).fill('#3b82f6');
    ...
    headers.forEach((header, index) => {
      doc.text(header, xPosition, yPosition + 8, { width: colWidths[index], align: 'center' });
      xPosition += colWidths[index];
    });
    ...
    data.data.forEach((item: any, rowIndex: number) => {
      if (yPosition > 700) {
        doc.addPage();
        yPosition = 50;
      }
      ...
      const rowColor = rowIndex % 2 === 0 ? '#f8fafc' : '#ffffff';
      doc.rect(tableStartX, yPosition, tableWidth, 18).fill(rowColor).stroke('#e2e8f0');
      
      xPosition = tableStartX + 5;
      ...
      // Format values to match reference document exactly
      const values = [
        String(item.station || 'N/A'),
        currentRks.toString(),
      ...
      values.forEach((value, index) => {
        const alignment = index === 0 ? 'left' : 'center'; // Station name left-aligned, others centered
        doc.text(String(value), xPosition, yPosition + 5, { width: colWidths[index], align: alignment });
        xPosition += colWidths[index];
      });
    ...
    yPosition += 5;
    doc.rect(tableStartX, yPosition, tableWidth, 20).fill('#1e40af');
    ...
    totalValues.forEach((value, index) => {
      ...
      doc.text(String(value), xPosition, yPosition + 6, { width: colWidths[index], align: alignment });
      xPosition += colWidths[index];
    });
    ...
    console.error("Error in generateStationComparativeLoadingPDF:", error);
    throw error;
  }
}
...
export function registerRoutes(app: Express): Server {
  ...
  // Custom Export PDF with Column Selection
  app.post("/api/exports/custom-export-pdf", async (req, res) => {
    try {
      if (!req.isAuthenticated()) {
        return res.status(401).send("Not authenticated");
      }

      const { data, columns, reportTitle, reportType } = req.body;
      ...
      const title = reportTitle || `Custom ${reportType || 'Data'} Export`;
      generateCustomExportPDF(doc, data, columns, title);

      doc.end();
    } catch (error: any) {
      console.error("Error generating custom export PDF:", error);
      res.status(500).json({
        error: "Failed to generate custom export PDF",
        details: error.message
      });
    }
  });
...
function generateCustomExportPDF(doc: typeof PDFDocument, data: any[], columns: string[], reportTitle: string) {
  // Add Railway Logo
  try {
    doc.image('./attached_assets/Indian_Railway_Logo_2_1750768462355.png', 50, 50, { width: 50, height: 50 });
  } catch (error) {
    console.log('Logo not found, proceeding without it');
  }

  // Header
  doc.fontSize(16).font('Helvetica-Bold').text('SOUTH CENTRAL RAILWAY', 120, 60);
  doc.fontSize(14).text(reportTitle, 120, 80);
  doc.fontSize(10).text(`Generated on: ${new Date().toLocaleDateString('en-IN')}`, 120, 100);

  let y = 140;
  
  // Calculate column widths dynamically
  const availableWidth = 720; // Total available width in landscape
  ...
  columns.forEach((column, index) => {
    ...
    doc.text(columnLabel, x, y, { width: columnWidth - 5 });
    x += columnWidth;
  });
  
  // Draw header underline
  doc.moveTo(50, y + 15).lineTo(770, y + 15).stroke();
  y += 25;
  ...
  data.forEach((row, rowIndex) => {
    ...
    columns.forEach((column) => {
      ...
      if (typeof value === 'number') {
        if (column.includes('MT') || column.includes('tonnage')) {
          value = (value / 1000).toFixed(3); // Convert to thousands
        } else if (column.includes('Avg') || column.includes('avgDay') || column.includes('avgPerDay')) {
          value = value.toFixed(3);
        } else {
          value = value.toLocaleString();
        }
      }
      doc.text(value.toString(), x, y, { width: columnWidth - 5 });
      x += columnWidth;
    });
    y += 12;
  });
  ...
  // Footer
  doc.fontSize(8).text('Generated by SCR Operations Management System', 50, 570);
  doc.text(`Page 1`, 720, 570);
}
...
function getColumnLabel(columnKey: string): string {
  const columnLabels: { [key: string]: string } = {
    'commodity': 'Commodity',
    'station': 'Station',
    'currentRks': 'Current RKs',
    'currentAvgDay': 'Current Avg/Day',
    'currentAvgPerDay': 'Current Avg/Day',
    'currentWagon': 'Current Wagons',
    'currentMT': 'Current MT',
    'currentFreight': 'Current Freight',
    'compareRks': 'Previous RKs',
    'compareAvgDay': 'Previous Avg/Day',
    'compareAvgPerDay': 'Previous Avg/Day',
    'compareWagon': 'Previous Wagons',
    'compareMT': 'Previous MT',
    'compareFreight': 'Previous Freight',
    'variationUnits': 'Variation Units',
